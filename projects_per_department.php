    ...
    <div class="col-12">
        <div class="card card-outline card-primary shadow rounded-0">
            <div class="card-body rounded-0">
                ...
                $students = $conn->query("SELECT * FROM `student_list` where id in (
                    SELECT a.student_id FROM archive_list a 
                    LEFT JOIN student_list s ON a.student_id = s.id 
                    WHERE a.`status` = 1 
                    AND COALESCE(NULLIF(a.curriculum_id,0), s.department_id) = '{$id}'
                )");
                $student_arr = ($students && $students->num_rows) ? array_column($students->fetch_all(MYSQLI_ASSOC),'email','id') : [];
                $cnt_q = $conn->query("SELECT COUNT(*) AS c FROM archive_list a LEFT JOIN student_list s ON a.student_id = s.id where a.`status` = 1 and COALESCE(NULLIF(a.curriculum_id,0), s.department_id) = '{$id}'");
                $count_all = ($cnt_q && ($r = $cnt_q->fetch_assoc())) ? (int)$r['c'] : 0;
                $pages = max(1, ceil($count_all/$limit));
                $archives = $conn->query("SELECT a.* FROM archive_list a LEFT JOIN student_list s ON a.student_id = s.id where a.`status` = 1 and COALESCE(NULLIF(a.curriculum_id,0), s.department_id) = '{$id}' order by unix_timestamp(a.date_created) desc {$paginate}");    
                ?>
                <div class="list-group">
                    ...
                    <?php endwhile; ?>
                    <?php else: ?>
                    <div class="text-center text-muted py-4">No projects found for this department.</div>
                    <?php endif; ?>
                </div>
            </div>
