...
if (!empty($videoPath)) {
  // Detect extension from URL (ignoring query strings) to determine MIME type
$cleanPath = parse_url($videoPath, PHP_URL_PATH);
$videoExt = strtolower(pathinfo($cleanPath, PATHINFO_EXTENSION));

if ($videoExt === 'mp4') $mimeType = 'video/mp4';
elseif ($videoExt === 'webm') $mimeType = 'video/webm';
elseif ($videoExt === 'ogg') $mimeType = 'video/ogg';
// Default is already video/mp4
}
...
if (isset($opts['coverImage']) && !empty($opts['coverImage'])) {
  $styleVars[] = 'background-image: url(\'' . $opts['coverImage'] . '\') !important';
  $styleVars[] = 'background-size: cover !important';
  $styleVars[] = 'background-position: center center !important';
  $styleVars[] = 'background-repeat: no-repeat !important';
  // Ensure background color doesn't override or show through strangely
  $styleVars[] = 'background-color: transparent !important'; 
}
...
<section class="<?php echo htmlspecialchars($classAttr); ?>"<?php echo $styleAttr; ?>>
  <?php if (!empty($videoPath)): ?>
    <video class="hero-video-element" 
           ...
           <?php echo (isset($opts['coverImage']) && !empty($opts['coverImage'])) ? 'poster="' . htmlspecialchars($opts['coverImage']) . '"' : ''; ?>
           playsinline>
