                ...
                <div class="container-fluid">
                    <table class="table table-hover table-striped">
                        ...
                        <tbody>
                            <?php 
                                $i = 1;
                                $curriculum = $conn->query("SELECT * FROM department_list where id in (SELECT curriculum_id from `archive_list` where student_id = '{$_settings->userdata('id')}' )");
                                $cur_arr = ($curriculum && $curriculum->num_rows) ? array_column($curriculum->fetch_all(MYSQLI_ASSOC),'name','id') : [];
                                $stu_prog = $conn->query("SELECT d.name as pname FROM student_list s LEFT JOIN department_list d ON s.department_id = d.id WHERE s.id = '{$_settings->userdata('id')}'");
                                $self_prog = ($stu_prog && $stu_prog->num_rows) ? ($stu_prog->fetch_assoc()['pname'] ?? '') : '';
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?php echo isset($cur_arr[$row['curriculum_id']]) ? $cur_arr[$row['curriculum_id']] : (!empty($self_prog) ? $self_prog : 'N/A') ?></td>
